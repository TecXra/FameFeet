            ...
            <tbody>
               {{-- {{ $blockUsers }} --}}
                ...
                <td>
                  ...
                  <td>{{ $following->phone_number }}</td>
                  <td>{{date("F j, Y", strtotime($following->date_of_birth))}}</td>

                    @if ($following->user_type == 1)
                      <td>Celebrity</td>
                      @elseif ($following->user_type == 2)
                      <td>Fan</td>
                      @elseif ($following->user_type == 9)
                      <td>Admin</td>
                    @endif

                    <td style="text-align:center">
                        <a href="{{ route('celebrity.unfollow',$following->id) }}"
                       style="color: red; font-size: 16px;
                        ...
                        {{-- <td><a href="{{ route('user.status',$following->id) }}" class="btn-sm btn-danger">InActive</a></td> --}}
