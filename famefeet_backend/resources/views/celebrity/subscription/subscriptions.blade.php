          ...
          <table class="table tablesorter ">
            <thead class=" text-primary">
              <tr>
                ...
                <th>Subscription Creation Date</th>
            ...
            <tbody>
                @foreach ($subscriptions as $subscription)
                  ...
                  <tr>
                        <td>
                            <img src="/{{ $subscription->celebrity->user->avatar }}" alt=""
                                style="border-radius: 5.2857rem;
                                width: 40px;">
                            {{$subscription->celebrity->user->username}}
                        </td>
                        <td>{{ $subscription->coins }} FameFeet Coins</td>
                        <td>{{date("F j, Y, g:i a", strtotime($subscription->created_at))}}</td>
                        {{-- <td>{{ $subscribeUser->subscription_end_date }}</td> --}}
                        ...
                        @if ($subscription->status == 1 )
                            <td><a href="{{ route('celebrity.subscription.status',$subscription->id) }}"
