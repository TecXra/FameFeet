          ...
          <table class="table tablesorter " id="">
            ...
            <tbody>
               {{-- {{ $blockUsers }} --}}
                ...
                <td>
                    <img src="/{{ $report->reported->avatar }}" alt=""
                  ...
                  <td>
                    {{ $report->reported->phone_number }}
                  </td>
                  <td>
                     {{(Str::length($report->message) > 50) ? Str::substr($report->message, 0,50).'...':$report->message}}
                  </td>
                    @if ($report->reported->user_type == 1)
                      <td>Celebrity</td>
                      @elseif ($report->reported->user_type == 2)
                      <td>Fan</td>
                      @elseif ($report->reported->user_type == 9)
                      <td>Admin</td>
                    @endif

                    <td style="text-align: center">
                    <a href="{{ route('celebrity.remove.report.user',$report->reported->id) }}"
                       style="color: red; font-size: 16px;
                        ...
                        {{-- <td><a href="{{ route('user.status',$blockUser->id) }}" class="btn-sm btn-danger">InActive</a></td> --}}
