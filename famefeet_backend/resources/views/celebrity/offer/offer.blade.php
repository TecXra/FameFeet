  ...
  <div class="col-md-12">
    ...
    <div class="card ">
      <div class="card-header">
        <h4 class="card-title">All Offers Of Fans</h4>
      </div>
      <div class="card-body">
        <div class="table-responsive">
          <table class="table tablesorter ">
            ...
            <tbody>
                ...
                <td>{{ $offer->status }}</td>
                <td>
                    @if(isset($offer->fan->user->avatar))
                      <img src="/{{ $offer->fan->user->avatar }}" alt=""
                      style="border-radius: 5.2857rem;
                      width: 30px;">
                    @endif
                    {{ $offer->fan->user->username }}
                </td>
                <td>{{date("F j, Y, g:i a", strtotime($offer->created_at))}}</td>
                <td>
                    <a href="{{ route('celebrity.offer.edit',$offer->id) }}"
                    ...
                    <a href="{{ route('celebrity.offer.show',$offer->id) }}"
