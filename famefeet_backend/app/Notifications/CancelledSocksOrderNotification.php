...
class CancelledSocksOrderNotification extends Notification
{
    use Queueable;
    public $subOrder;
    public $user;
    ...
    public function toVonage($notifiable){
        return (new VonageMessage())
            ->content($this->subUser->username.' your order has been cancelled due to inactivity for longer
            than 2 days. Go to '.url('/').'now to check! Thank you for using FameFeet')->from('Famefeet');
    }
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ->line('Due to policy, we have cancelled your sock order. You failed to either accept the order or upload the tracking number within 48 hours.')
